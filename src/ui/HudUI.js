        ...
        update({ gained, total, base, mult }) {
            ...
            return this._enqueue(() => this._runSteps([
                ...
                [
                    ...
                    () => { this._gainedVal.classList.add('scoring'); return this._setAndPulse(this._gainedVal, gained > 0 ? `${gained}` : ''); },
                ],
                () => this._shakeDigits(this._gainedVal, 0.5),
                () => this._sleep(500),
                [
                    () => { this._onScoreAccum?.(); this._stopShakeDigits(this._gainedVal); return this._animateCount(this._gainedVal, gained, 0, Math.min(gained*10, 200)).then(() => this._gainedVal.classList.remove('scoring')); },
